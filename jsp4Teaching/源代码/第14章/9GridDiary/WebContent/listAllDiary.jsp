...
function zoom(id,url){
	...
	document.getElementById("diary"+id).style.cursor="url(images/ico02.ico),auto";//为图片添加自定义鼠标样式
		document.getElementById("control"+id).style.display="";	//显示控制工具栏
		document.getElementById("diaryImg"+id).style.width="401px";	//设置日记图片的宽度
		document.getElementById("diaryImg"+id).style.height="436px";	//设置日记图片的高度
		document.getElementById("canvas"+id).style.cursor="url(images/ico02.ico),auto";	//为画布添加自定义鼠标样式
		...
		document.getElementById("diary"+id).style.cursor="url(images/ico01.ico),auto";//为图片添加自定义鼠标样式
		document.getElementById("diaryImg"+id).style.width="60px";	//设置日记图片的宽度
		document.getElementById("diaryImg"+id).style.height="60px";	//设置日记图片的高度		
		document.getElementById("canvas"+id).style.cursor="url(images/ico01.ico),auto";	//为画布添加自定义鼠标样式
...
function rotate${id.count }(){
	...
	<div style="margin:10px 10px 0px 10px;background-color:#FFFFFF; border-bottom-color:#CBCBCB;border-bottom-style:dashed;border-bottom-width: 1px;">
		<div id="diaryImg${id.count }" style="border:1px #DDDDDD solid;width:60px;background-color:#EEEEEE;">
			...
			<div id="control${id.count }" style="display:none;padding: 10px;width:401px;">
			...
			<img id="diary${id.count }" src="images/diary/${diaryList.address }scale.jpg" 
				style="cursor: url(images/ico01.ico),auto;" 
