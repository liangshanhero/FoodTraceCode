...
function checkUser(str){
	if(str==""){			//当用户名为空时
		...
		$.get("UserServlet?action=checkUser&username="+encodeURIComponent(str)+"&nocache="+new Date().getTime(),function(data){
			if(data=="1"){											//当用户名没有被注册
				document.getElementById("div_user").innerHTML="";		//清空提示文字
				document.getElementById("tr_user").style.display='none';		//隐藏提示信息显示行
				flag_user=true;		
			}else{												//当用户名已经被注册
				document.getElementById("div_user").innerHTML=data;		//设置提示文字
				document.getElementById("tr_user").style.display='block';		//显示提示信息
				flag_user=false;
			}			
		});
...
function save(){
	...
	if(flag_user && flag_pwd && flag_repwd && flag_email && flag_question && flag_answer){	//所有数据都符合要求时
		...
		$.ajax({
			url:"UserServlet?action=save&nocache="+new Date().getTime(),
			type:"POST",
			data:param,
...
function getProvince(){
	$.get("UserServlet?action=getProvince&nocache="+new Date().getTime(),function(data){
		provinceArr=data.split(",");//分割字符串
...
function getCity(selProvince){
	$.get("UserServlet?action=getCity&parProvince="+encodeURIComponent(selProvince)+"&nocache="+new Date().getTime(),function(data){
	cityArr=data.split(",");//分割字符串
	$("#city").empty();	//清空下拉列表
...
function form_reset(form){
  ...
  <div id="register" style="width:663px; height:426px;margin-left:68px; background-color:#546B51; padding:4px; position:absolute; z-index:11;display:none;">
