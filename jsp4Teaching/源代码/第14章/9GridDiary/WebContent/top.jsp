...
function myClose(divID){
	document.getElementById(divID).style.display='none';			//设置id为login的层隐藏
...
function loginSubmit(form2){
	...
	$.ajax({
		url:"UserServlet?action=login",		//设置请求地址
		type : "POST", 								//设置请求方式
		dataType : "html", 							//设置返回数据的类型
		data:param,										//设置传递的数据
		//设置请求成功时执行的回调函数
		success : function(data) {
			data=data.replace(/\s/g,"");	//去除字符串中的Unicode空白
			alert(data);
			if(data=="登录成功！"){
...
<form name="form2" method="post" action="" id="form2">
	...
	<ul id="loginUl"><li>
	用户名称：<input type="text" name="username" style="width:120px" onkeydown="if(event.keyCode==13){this.form.pwd.focus();}">
	</li><li>
